...
use crate::{service, expand_streams, event_loop, check_break, feed};
...
)
{
	event_loop!
	{
		...
		{
			Ok (item_string) => check_break! (feed! (string_sink, item_string)),
...
)
{
	event_loop!
	{
		...
		{
			Ok (item) => check_break! (feed! (item_sink, item)),
			Err (serde_error) => event!
			(
				Level::ERROR,
